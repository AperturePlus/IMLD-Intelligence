...
import xenosoft.imldintelligence.module.community.internal.model.CommunityPostLike;

@Mapper
public interface CommunityPostLikeMapper extends BaseMapper<CommunityPostLike> {

    @Insert("INSERT INTO community_post_like (tenant_id, post_id, toc_user_id) " +
    ...
    @Delete("DELETE FROM community_post_like WHERE tenant_id = #{tenantId} AND post_id = #{postId} AND toc_user_id = #{tocUserId}")
